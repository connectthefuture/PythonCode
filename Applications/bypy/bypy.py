...
class ByPy(object):
    ...
    def __dump_exception(self, ex, url, pars, r, act):
        if self.Debug or self.Verbose:
            ...
            if hasattr(r, 'status_code'):
                ...
                if (r.status_code != 200 and r.status_code != 206) or (
                        not (pars.has_key('method') and pars['method'] == 'download') and url.find(
                            'method=download') == -1 and url.find('baidupcs.com/file/') == -1):
    ...
    def __request_work(self, url, pars, act, method, actargs=None, addtoken=True, dumpex=True, **kwargs):
        ...
        try:
            ...
            if sc == requests.codes.ok or sc == 206:  # 206 Partial Content
                if sc == requests.codes.ok:
                    if pars.get('method') != 'download':
                        try:
                            j = r.json()
                            if j.get('error_code') == 0 and j.get(
                                    'error_msg') == u'no error':  # __walk_remote_dir_act() KeyError: u'list'
            ...
            elif sc == 404 and r.url.find(
                    'http://bcscdn.baidu.com/bcs-cdn/wenxintishi') == 0:  # = "error_code":31390,"error_msg":"Illegal File"
...
'''
    ...
    def __panapi_unzipcopy_file_act(self, r, args):
        ...
        if 'path' in j:
            ...
            if j['error_code'] == 31196:
                perr("Remote extract: '{}#{}' =xx=> '{}' FAILED. File already exists.".format(args['path'],
                                                                                              args['subpath'],
                                                                                              args['topath']))
                ...
                perr("Remote extract: '{}#{}' =xx=> '{}' FAILED. File too large.".format(args['path'], args['subpath'],
                                                                                         args['topath']))
                return EMaxRetry
            else:
                perr("Remote extract: '{}#{}' =xx=> '{}' FAILED. Unknown error {}: {}.".format(args['path'],
                                                                                               args['subpath'],
                                                                                               args['topath'],
                                                                                               j['error_code'],
                                                                                               j['error_msg']))
    ...
    def __panapi_unzipcopy_file(self, rpath, subpath, topath):
        ...
        return self.__get(pcsurl + 'file',
                          pars, self.__panapi_unzipcopy_file_act, addtoken=False, cookies=self.__pancookies,
                          actargs=pars)
        ...
        if self.__current_file_size > MinRapidUploadFileSize:
            ...
            if result == ENoError:
                ...
                if not self.__rapiduploadonly:
                    ...
                    self.pv(
                        "'{}' can't be rapidly uploaded, so it's skipped since we are in the rapid-upload-only mode.".format(
                            localpath))
    ...
    def __downchunks(self, rfile, start):
        ...
        while True:
            ...
            if headers.has_key('Range'):
                self.pd("headers['Range'][6:]: {} {}".format(headers['Range'][6:],
...
'''
        ...
        parser.add_argument("-d", "--debug", dest="debug", action="count", default=0,
                            help="enable debugging & logging [default: %(default)s]")
        ...
        parser.add_argument("-r", "--retry", dest="retry", default=5,
                            help="number of retry attempts on network error [default: %(default)i times]")
        parser.add_argument("-q", "--quit-when-fail", dest="quit", default=False,
        ...
        parser.add_argument("-s", "--slice", dest="slice", default=DefaultSliceSize,
                            help="size of file upload slice (can use '1024', '2k', '3MB', etc) [default: {} MB]".format(
                                DefaultSliceInMB))
        parser.add_argument("--chunk", dest="chunk", default=DefaultDlChunkSize,
                            help="size of file download chunk (can use '1024', '2k', '3MB', etc) [default: {} MB]".format(
                                DefaultDlChunkSize / OneM))
        parser.add_argument("-e", "--verify", dest="verify", action="store_true", default=False,
                            help="Verify upload / download [default : %(default)s]")
        parser.add_argument("-f", "--force-hash", dest="forcehash", action="store_true",
                            help="force file MD5 / CRC32 calculation instead of using cached value")
        parser.add_argument("-l", "--list-file", dest="listfile", default=None,
                            help="input list file (used by some of the commands only [default: %(default)s]")
        parser.add_argument("--resume-download", dest="resumedl", default=True,
                            help="resume instead of restarting when downloading if local file already exists [default: %(default)s]")
        parser.add_argument("--include-regex", dest="incregex", default='',
        ...
        parser.add_argument("--on-dup", dest="ondup", default='overwrite',
                            help="what to do when the same file / folder exists in the destination: 'overwrite', 'skip', 'prompt' [default: %(default)s]")
        parser.add_argument("--no-symlink", dest="followlink", action="store_false",
                            help="DON'T follow symbol links when uploading / syncing up")
        parser.add_argument(DisableSslCheckOption, dest="checkssl", action="store_false",
        ...
        parser.add_argument("--mirror", dest="mirror", default=None,
                            help="Specify the PCS mirror (e.g. bj.baidupcs.com. Open 'https://pcs.baidu.com/rest/2.0/pcs/manage?method=listhost' to get the list) to use.")
        parser.add_argument("--rapid-upload-only", dest="rapiduploadonly", action="store_true",
                            help="Only upload large files that can be rapidly uploaded")

        # action
        parser.add_argument(CleanOptionShort, CleanOptionLong, dest="clean", action="count", default=0,

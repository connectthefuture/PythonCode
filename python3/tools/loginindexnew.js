...
$(document).ready(function () {
    if (!IsLogined) {
        ...
        $.get("/ajax/indexinfo.ashx", function (data, status) {
            var userinfo = $.parseJSON(data);
            var loginedHtml = "<a href='/user/" + userinfo.userid + ".html' class='photo'><img src='http://122.228.213.115/qingrenw.com/userimage/small/";
            if (userinfo.photo == "") {
                if (userinfo.sex == 0) loginedHtml += "default_girl.jpg";
                else loginedHtml += "default_man.jpg";
            }
            else {
                loginedHtml += userinfo.photo;
            }

            loginedHtml += "' /></a><ul><li>欢迎您,<a href='/user/" + userinfo.userid + ".html' class='yellow'>" + userinfo.nickname + "</a></li><li>";
            if (userinfo.vip == "0") {
                loginedHtml += "普通会员";
            }
            ...
            loginedHtml += "</li><li>当前情人豆: <span  class='yellow'>" + userinfo.goldnum + "</span> 颗</li>";
            loginedHtml += "<li><a href='/user/info/baseinfo.aspx' class='yellow'>完善个人资料</a></li></ul>";
            loginedHtml += "<a href='/user/myself.aspx' class='center'>进入个人中心</a>";
            loginedHtml += "<center><a href='/control/loginout.ashx' class='yellow'>安全退出</a></center>";
            $("#nologin").hide();
            $("#logined").show().html(loginedHtml);
        });
    }
    $("#newcontent").hide();
    ...
    $("#kinMaxShow").kinMaxShow({
        ...
        button: {
            showIndex: false,
            normal: {
                background: 'url(images/index/button.png) no-repeat -14px 0',
                marginRight: '8px',
                border: '0',
                right: '44%',
                bottom: '110px'
...
function doLogin() {
    if ($("#UserName").val() == "") {
        $.tooltip("请输入登录邮箱或ID", 5000, false);
    }
    else if ($("#UserPwd").val() == "") {
        $.tooltip("请先填密码", 5000, false);
    }
    else {
        $.showLoading("正在登录……");
        $.post("/control/checklogin.ashx", {
            ...
            $.hideLoading();
            if (result.state == 1) {
                window.location.href = "/user/myself.aspx";
            }
            else {
                $.tooltip(result.notice, 5000, false);
            }
        });
    }
}
var page = 1;
function getNewPushUser() {
    $.get("/Control/IndexUser.ashx?page=" + page++, function (data, status) {
        var result = $.parseJSON(data);
        var arr = result.data;
        if (arr.length > 0) {
            for (var i = 0; i < arr.length; i++) {
                $("#indexUser").append('<li><a href="/user/' + arr[i].uid + '.html" class="img"><img src="http://122.228.213.110/qingrenw.com/userimage/big/' + arr[i].photo + '" /></a><p><label>昵称</label>/<a href="/user/' + arr[i].userid + '.html">' + arr[i].nickname + '</a></p><p><label>年龄</label>/' + arr[i].age + '岁 &nbsp;&nbsp;&nbsp;&nbsp;<label>学历</label>/' + arr[i].edu + '</p></li>');
            }
        }
        else {
            $.tooltip("暂时无法推荐更多用户", 5000, false);
        }
    });
}